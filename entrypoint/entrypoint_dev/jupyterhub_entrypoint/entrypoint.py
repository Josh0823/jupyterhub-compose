...
from .api import APIUserSelectionHandler
...
"""
Need to setup a handler to add new entrypoints
fetch envs, lists, and scripts from a different api
"""
...
class EntrypointService(Application, Configurable):
    ...
    def initialize(self, argv=None):
        ...
        self.settings = {
            ...
            "storage": FileStorage(os.path.join(self.storage_path, "{user[0]}/{user}.json"))
        }
        ...
        self.app = web.Application([
            ...
            (self.service_prefix + r"images/user/(.+)", APIShifterImageHandler, dict(shifter_api_token = self.shifter_api_token, shifter_api_host = self.shifter_api_host))
        ], **self.settings)
